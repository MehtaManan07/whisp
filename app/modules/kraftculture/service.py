...
class KraftcultureService:
    ...
    def _is_email_processed_sync(self, db: Session, gmail_message_id: str) -> bool:
        result = db.execute(
            ...
            for email in emails:
                try:
                    ...
                    already_processed = await run_db(
                        lambda db, eid=email.id: self._is_email_processed_sync(db, eid)
                    )
                    ...
                    for number in self.whatsapp_numbers:
                        try:
                            ...
                            sent_count += 1
                            whatsapp_sent = True
                            logger.info(f"Sent email '{email.subject}' to {number}")
                    ...
                    _ws = whatsapp_sent
                    await run_db(
                        lambda db, e=email, p=_parsed, w=_ws: self._save_order_email_sync(db, e, p, w)
                    )
